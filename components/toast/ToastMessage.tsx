...
export default function ToastMessage({id, header, message, lifetime, onRemove, truncate = 'truncate-1-lines', icon, type}: ToastMessage) {
 const Var = type
  ? VARIANTS[type]
  : {
  ...
  >
   <div className="flex flex-row p-2 flex-no-wrap w-full">
    ...
    <div className="flex flex-col flex-no-wrap px-1 w-full">
     ...
     <p className={clsx('-mt-0.5 my-auto break-all flex', 'text-gray-600 text-sm', typeof message === 'string' && truncate)}>{message}</p>
    </div>
    <div onClick={() => onRemove && onRemove(id)} className={clsx('w-10 h-12 mr-2 items-center mx-auto', 'text-center leading-none text-lg flex')}>
     <FontAwesomeIcon className={clsx('mx-auto my-auto text-center text-gray-600', 'cursor-pointer hover:scale-105 transform ')} icon={faTimes} />
    </div>
   </div>
  </div>
 );
